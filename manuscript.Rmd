...
```{r cache = TRUE}
# Conduct equivalent analyses the Bayesian way
agg <- aggregate(SRI ~ material + block + id, data = tmp, FUN = mean)
agg$id <- factor(agg$id)

bf_rts <- anovaBF(
  formula = SRI ~ material * block + id
  , data = agg
  , whichRandom = "id"
  , whichModels = "top"
)

apa_bf_rts <- apa_print(bf_rts, reciprocal = TRUE)
```

Conducting a similar 2 (*material*: permuted vs. random) $\times$ 6 (*block number*) Bayesian ANOVA,
we found an effect of *block*,
`r apa_bf_rts$block`,
indicating that response times decreased over blocks.
We did not find strong evidence for a main effect of *material*,
`r_apa_bf_rts$material`.
<!-- What about the interaction term? I'm lost!!! -->
